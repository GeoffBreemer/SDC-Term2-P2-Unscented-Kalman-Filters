...
******************************************************************************/
  ...
  for (int i = 1; i <= n_x; i++)
  {
    ...
    Xsig.col(i+n_x) = x - sqrt(lambda+n_x) * A.col(i-1);
  }
 ...
 ******************************************************************************/
  ...
  MatrixXd prod;
  for(int i=0;i<2 * n_aug + 1;i++) {
    prod = Xsig_pred.col(i) - x;

    // normalise psi to be between -pi and pi
    while (prod(3)> M_PI) prod(3)-=2.*M_PI;
    while (prod(3)<-M_PI) prod(3)+=2.*M_PI;

    P += weights(i) * prod * prod.transpose();
  }
 ...
 ******************************************************************************/
  ...
  for(int i=0;i<2 * n_aug + 1;i++) {
    ...
    while (prod(1)> M_PI) prod(1)-=2.*M_PI;
    while (prod(1)<-M_PI) prod(1)+=2.*M_PI;

    S += weights(i) * prod * prod.transpose();
  }
 ...
 ******************************************************************************/
  ...
  Tc.fill(0);
  for (int i=0;i<2 * n_aug + 1;i++) {
    MatrixXd first = Xsig_pred.col(i) - x;
    while (first(3)> M_PI) first(3)-=2.*M_PI;
    while (first(3)<-M_PI) first(3)+=2.*M_PI;

    MatrixXd second = Zsig.col(i) - z_pred;
    while (second(1)> M_PI) second(1)-=2.*M_PI;
    while (second(1)<-M_PI) second(1)+=2.*M_PI;

    Tc += weights(i) * first * second.transpose();
  }
